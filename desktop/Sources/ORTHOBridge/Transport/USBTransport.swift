...
public final class USBTransport: ORTHOTransport, @unchecked Sendable {
    public let deviceIdentifier: String
    public var name: String { "USBTransport[\(deviceIdentifier)]" }
    ...
    public func connect() throws {
        ...
        // Windows: WinUSB Initialize, claim interface, pipe policy
        _isConnected = true
    }
    ...
    public func submit(_ command: ORTHOCommand) throws -> ORTHOCompletion {
        ...
        // USB bulk transfer: command -> endpoint OUT, completion <- endpoint IN
        let cycles: UInt64
        ...
        let rh = ORTHOCommand.hashHex(Data("usb-\(command.sequence)".utf8))
        let tr = ORTHOCommand.hashHex(Data("usb-trace-\(command.sequence)".utf8))
